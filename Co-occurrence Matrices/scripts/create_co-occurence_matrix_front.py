    ...
    if line.startswith("HICO"):
        ...
        for key,value in dict_hum.items():
            ...
            for entry in actions_local_list:
                if actions_global_dict_front.get(entry) is None:
                    actions_global_dict_front[entry]=[]
            ...
            for i in range(len(object_id_list)):
                ...
                for key,value1 in actions_global_dict_front.items():
                    ...
                    if(action==key):
                        plus_x=1 if "+x" in front else 0
                        minus_x=1 if "-x" in front else 0
                        plus_y=1 if "+y" in front else 0
                        minus_y=1 if "-y" in front else 0
                        plus_z=1 if "+z" in front else 0
                        minus_z=1 if "-z" in front else 0
                        n_a=1 if "n/a" in front else 0
                        
                        if(not is_duplicate):
                           dict1 = {obj_name: {"+x":plus_x,"-x":minus_x,"+y":plus_y,"-y":minus_y,"+z":plus_z,"-z":minus_z,"n/a":n_a}}
                           actions_global_dict_front[action].append(dict1)    
                        else:
                            for dict in value1:
                                if(obj_name in dict.keys()):
...
actions_front_array=np.zeros((len(unique_actions_orderedSet),len(unique_objects_orderedSet),7))#(depth,rows,columns)

depth=-1
row=-1
for key,value in actions_global_dict_front.items():
    depth+=1
    for dict1 in value:       
        for key1,value1 in dict1.items():
            ...
            for entry in unique_objects_orderedSet:
                row+=1
                if(key1==entry):
                    actions_front_array[depth][row][0]=value1["+x"]
                    actions_front_array[depth][row][1]=value1["-x"]
                    actions_front_array[depth][row][2]=value1["+y"]
                    actions_front_array[depth][row][3]=value1["-y"]
                    actions_front_array[depth][row][4]=value1["+z"]
                    actions_front_array[depth][row][5]=value1["-z"]
                    actions_front_array[depth][row][6]=value1["n/a"]
                    
         
#print("*****actions_front_array*******")
#print(actions_front_array)

print("-------------------------------------------------------------")
print("***** actions_global_dict_front *******")
print("-------------------------------------------------------------")
print(actions_global_dict_front)
...
actions_front_cond_prob=np.zeros((len(unique_actions_orderedSet),len(unique_objects_orderedSet),7))#(depth,rows,columns)
actions_front_joint_prob=np.zeros((len(unique_actions_orderedSet),len(unique_objects_orderedSet),7))#(depth,rows,columns)
...
total_sum_array=actions_front_array.sum()
count_joint_prob= total_sum_array if total_sum_array!=0 else 1
for i in range(len(actions_front_array)):
    for j in range(len(actions_front_array[i])):
        sum_row=sum(actions_front_array[i][j])
        count_cond_prob= sum_row if sum_row!=0 else 1
        for k in range(len(actions_front_array[i][j])):
            cond_prob  = actions_front_array[i][j][k]/count_cond_prob
            joint_prob = actions_front_array[i][j][k]/count_joint_prob
            actions_front_cond_prob[i][j][k]  = cond_prob
            actions_front_joint_prob[i][j][k] = joint_prob

f_actions_front_array = open("actions_front_array.txt", "a")
f_actions_front_cond_prob = open("actions_front_cond_prob.txt", "a")
f_actions_front_joint_prob = open("actions_front_joint_prob.txt", "a")
...
print("-------------------------------------------------------------")
#print(actions_front_array)
f_actions_front_array.write(str(actions_front_array))

print("-------------------------------------------------------------")
print("***** Calculate Conditional Probabilities *******")
print("-------------------------------------------------------------")
print("-------------------------------------------------------------")
print("***** actions_front_cond_prob array *******")
print("-------------------------------------------------------------")
#print(actions_front_cond_prob)
f_actions_front_cond_prob.write(str(actions_front_cond_prob))
...
print("-------------------------------------------------------------")
print("-------------------------------------------------------------")
print("*****actions_front_joint_prob array*******")
print("-------------------------------------------------------------")
print("Total Sum of elements of array is : ",total_sum_array)
#print(actions_front_joint_prob)
f_actions_front_joint_prob.write(str(actions_front_joint_prob))

f_actions_front_array.close()
f_actions_front_cond_prob.close()
f_actions_front_joint_prob.close()

pickle.dump(actions_front_array, open("actions_front_array.pkl", "wb"))
pickle.dump(actions_front_cond_prob, open("actions_front_cond_prob.pkl", "wb"))
pickle.dump(actions_front_joint_prob, open("actions_front_joint_prob.pkl", "wb"))