...
from model.model import initialize_bert_model
from model.dataset import HicoDetImgDescDataset
import configparser
import torch
...
def train_model(model, tokenizer, dataloaders, criterion, optimizer, num_epochs=25, device=torch.device("cpu"), run=None):
    since = time.time()
    ...
    for epoch in range(num_epochs):
        ...
        for data in tqdm(dataloaders["train"]):
            inputs = data["caption"]
            encoded_input = tokenizer(inputs, return_tensors='pt', padding=True, truncation=True)
            labels = data["label"]
            ...
            # statistics
            running_loss += loss.item() * encoded_input['input_ids'].size(0)
...
if __name__ == "__main__":
    ...
    # lr = 0.001
    # optimizer = "sgd"
    for model_name in ["bert-base-uncased", "bert-large-uncased", "roberta-base", "gpt2", "albert-base-v2"]:
        for lr in [0.01, 0.001, 0.0001]:
            for optimizer in ["sgd", "adam", "adamw", "adagrad"]:
                for desc_model in ["conceptual_weights.pt", "coco_weights.pt"]:
                    for beam in [True, False]:
                        for feature_extract in [True, False]:
                            if os.path.exists(f'{model_name}_{optimizer}_{lr}_{desc_model}_{beam}_{feature_extract}_weights.pth'):
                                continue

                            run = wandb.init(project="affordance_desc", config={"model_name": model_name, "lr": lr, "opimizer": optimizer,
                                                                           "desc_model": desc_model, "feature_extract": feature_extract}, reinit=True)

                            model_ft, tokenizer = initialize_bert_model(model_name, num_classes, feature_extract)
                            ...
                            image_datasets = {"train": HicoDetImgDescDataset(config=configp,train=True, model=desc_model, beam=beam),
                                              "val": HicoDetImgDescDataset(config=configp, train=False, model=desc_model, beam=beam)}
                            ...
                            # Train and evaluate
                            model_ft, hist = train_model(model_ft, tokenizer, dataloaders_dict, criterion, optimizer_ft, num_epochs=num_epochs, device=device, run=run)

                            torch.save(model_ft.state_dict(), f'{model_name}_{optimizer}_{lr}_{desc_model}_{beam}_{feature_extract}_weights.pth')
                            torch.save(model_ft, f'{model_name}_{optimizer}_{lr}_{desc_model}_{beam}_{feature_extract}_weights.pth')
                            print(hist)
                            run.finish()