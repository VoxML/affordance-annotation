...
from utils import util_io as io


class ConvertMat2Json:
    def __init__(self, config):
        self.config = config
        self.anno = scio.loadmat(self.config["HICODET"]["anno_file"])
        self.anno_bbox = scio.loadmat(self.config["HICODET"]["anno_bbox_file"])
    ...
    def create_anno_list(self):
        ...
        for subset in ['train', 'test']:
            ...
            for i in tqdm(range(num_samples)):
                ...
                anno = {'global_id': global_id, 'image_path_postfix': f'{subset}2015/{image_jpg}',
                        'image_size': self.get_image_size(i, subset), 'hois': self.get_hoi_bboxes(i, subset),
                        # 'pos_hoi_ids': [str(k[0] + 1).zfill(3) for k in \
                        #                np.argwhere(self.anno[f'anno_{subset}'][:, i] == 1).tolist()],
                        # 'neg_hoi_ids': [str(k[0] + 1).zfill(3) for k in \
                        #                np.argwhere(self.anno[f'anno_{subset}'][:, i] == -1).tolist()]
                        }
    ...
    def convert(self):
        ...
        io.dump_json_object(anno_list, self.config["HICODET"]["anno_list_json"])

        print('Creating hoi list ...')
        hoi_list = self.create_hoi_list()
        io.dump_json_object(hoi_list, self.config["HICODET"]["hoi_list_json"])
        ...
        io.dump_json_object(object_list, self.config["HICODET"]["object_list_json"])
        ...
        io.dump_json_object(verb_list, self.config["HICODET"]["verb_list_json"])
...
if __name__ == '__main__':
    ...
    converter = ConvertMat2Json(configp)
    converter.convert()
