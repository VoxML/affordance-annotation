...
def merge_data(args):
    df = pd.read_csv(args.pair_data, index_col=0, converters={"h_bbox": literal_eval, "o_bbox": literal_eval})
    df["verbs"] = None
    ...
    for index, row in df.iterrows():
        ...
        for cluster in clusters:
            if check_in_cluster(cluster, (comp_1, comp_2)):
                for verb in verbs:
                    data.append({'file': file, 'cluster': cluster, 'verb': verb})

    df = pd.DataFrame.from_dict(data)
    #df = df.loc[df['cluster'] != ((-40, -40), (-10, 20))]
    df = df.loc[df['verb'] == "ride"]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pair_data', default='images_final/original/pair_data/UPT Pair Tokens TSNE Inst_500 anno Gibs_Telic.csv', type=str)
